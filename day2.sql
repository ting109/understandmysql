...
-- 查询s_id为101和102的记录
select s_id from fruits where s_id in (101,102);
-- 查询所有s_id不等于101也不等于102的记录
select s_id from fruits where not in (101,102);
...
select s_id from fruits where f_name like '____y';
...
select e.*,(select max(ei.e_salary) from employee ei where e.dept_no =  select e.*,(select max(ei.e_salary) from employee ei where e.dept_no = ei.dept_no) max from employee e;
-- 在employee表中，计算每个部门各有多少名员工。
select count(*),dept_no from employee group by dept_no;
-- 在employee表中，计算不同类型职工的总工资数。
select sum(e_salary) sum,e_job from employee group by e_job;
-- 
select avg(e_salary) avg,dept_no from employee group by dept_no;
...
select f_price,f_name from fruits where (s_id = 101 or s_id = 101) and f_price > 5 and f_name='apple';
...
select distinct s_id from fruits group by s_id; 
...
select f_name from fruits group by s_id; 
...
select s_id count(*) as num from fruits group by s_id;
